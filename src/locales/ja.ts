export const ja = {
  ...
  expect: {
    guide: {
      ...
      introduction: 'ダイス式を押すと期待値と確率分布を計算できます。',
      command: 'ダイス式',
      mean: '期待値',
      range: '出目の範囲',
      calculate: '{{- command}}を計算',
      formula: '公平なn個のm面ダイス（ndm）の期待値は n × (m + 1) ÷ 2。固定値はそのまま加算します。',
      '3d6-title': '3d6の期待値は10.5',
      '3d6-description': '6面ダイス3個の合計です。期待値は3 × 3.5 = 10.5、出目は3〜18です。',
      '3d6-distribution': '最も出やすい合計は10と11（各12.5%）。11以上が出る確率は50%です。',
    },
    usage1:
  ...
  'analyze-logs': {
    ...
    usage1: 'ココフォリアのログから、キャラクターごとの出目の平均・成功率・グラフを確認できます。',
    guide: {
      title: 'ココフォリアのログから出目を集計する方法',
      step1: 'ココフォリアのルームでログ出力を開き、HTML形式のログを保存します。',
      step2: 'このページでHTMLログを選択するか、ドラッグ＆ドロップします。',
      step3: 'タブ・ゲームシステム・キャラクターを選んで集計結果を確認します。',
      'systems-title': '対応するゲームシステム',
      systems:
        'クトゥルフ神話TRPG（6版）、新クトゥルフ神話TRPG（7版）、エモクロアTRPG、シノビガミ、ネクロニカ、ソード・ワールド2.5に対応しています。',
      interpretation: '成功率は、ログ内で成功・失敗を判定できるロールの実績です。',
