...
const Coc7thMatcher: Matcher = (message) => {
  ...
  const diceTarget = Number.parseInt(match[1], 10);
  const diceResultNumber = match[3].includes(',') ? undefined : Number.parseInt(match[4], 10);
...
const CoC6thMatcher: Matcher = (message) => {
  ...
  const diceTarget = Number.parseInt(match[1], 10);
  const diceResultNumber = Number.parseInt(match[2], 10);
...
export type SingleMessageParser = (message: string) => MatcherResult | undefined;
...
export type MessageParser = (message: string) => ReturnType<SingleMessageParser>[];
