...
const visibilityOptions: VisibilityOption[] = [
  { value: 'private', label: '非公開 (自分のみ)', icon: IconLock },
  { value: 'unlisted', label: '限定公開 (リンクを知っている人のみ)', icon: IconSparkles },
  { value: 'public', label: '公開 (一覧に表示)', icon: IconLockOpen },
];
...
export const useEditAnalysisDialog = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: '解析結果を更新できませんでした', variant: 'destructive' });
    }
  };
  ...
  const render = () => (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>解析結果を編集</DialogTitle>
          <DialogDescription>タイトルと公開設定を変更できます</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="edit-sessionDate">セッション日</Label>
            <Input
              id="edit-sessionDate"
              type="date"
              value={sessionDate}
