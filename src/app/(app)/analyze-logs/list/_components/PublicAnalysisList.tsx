...
export const PublicAnalysisList: FC = () => {
  ...
  if (error && visibleAnalyses.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 py-8 text-center">
        <div className="text-red-600 text-sm">解析結果を取得できませんでした</div>
  ...
  if (visibleAnalyses.length === 0) {
    return <div className="text-slate-500 text-sm py-8 text-center">検索結果が見つかりませんでした</div>;
  }
    ...
    <div className="relative">
      ...
      {showReloadOverlay && (
        <div className="absolute inset-0 z-10 grid place-content-center rounded-md bg-white/70 backdrop-blur-[1px]">
          <span className="text-slate-600 text-sm font-medium">読み込み中...</span>
        </div>
      )}
    </div>
  );
