...
type ParameterInputProps = Omit<ComponentPropsWithoutRef<'div'>, 'onChange'> & {
  ref?: Ref<CompositeFieldFocusHandle>;
  value: Parameter[];
  onChange: (value: Parameter[]) => void;
};

export const ParameterInput = ({ value, onChange, className, ref, ...rootProps }: ParameterInputProps) => {
  const handleParameterChange = variableFieldChangeHandlerGenerator<Parameter>(value, onChange);
  const { primaryRef, fallbackRef } = useCompositeFieldFocus<HTMLInputElement, HTMLButtonElement>(ref ?? null);
        ...
        {value.map((status, index) => (
          <div key={status.key} className="flex items-center gap-2">
            ...
            <Button
              ...
              aria-label={`${status.label}のパラメータを削除`}
              onClick={() => {
