      ...
      <div className="flex items-center gap-2 rounded-md bg-[#2b2a2a] p-4 ">
        ...
        <div className="space-y-1">
          <div>
            ...
            <span className="text-[#bdbdbd]"> - 今日 13:04</span>
          </div>
          <div className="text-white">これはテストメッセージです。キャラの名前に色が反映されます。</div>
        </div>
      </div>
...
const ColorTip: FC<ColorTipProps> = ({ color, value, onChange }) => {
  ...
  return (
    <Button
      type="button"
      variant="ghost"
      className="h-8 w-8 p-0 transition-opacity duration-75 hover:opacity-80"
      style={{ backgroundColor: color }}
      onClick={() => onChange(color)}
      aria-label={`チャットカラーに${color}を選択`}
      aria-pressed={value === color}
    >
      {value === color && <IconCheck size="16" className={clsx(isDarkColor ? 'text-white' : 'text-slate-700')} />}
    </Button>
  );
};
