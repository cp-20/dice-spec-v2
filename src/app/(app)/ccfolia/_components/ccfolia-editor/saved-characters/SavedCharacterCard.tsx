...
export const SavedCharacterCard: FC<SavedCharacterCardProps> = ({ character }) => {
  ...
  const updatedAt = character.updatedAt.toDate().toLocaleString('ja-JP', {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-xs text-muted-foreground">{`${updatedAt} 更新`}</span>
      </button>
      ...
      <div className="grid grid-cols-2 border-t">
        <Button
          ...
          disabled={exporting}
          aria-label={exported ? `コピーしました: ${character.name}` : `エクスポート: ${character.name}`}
        >
      ...
      {exported && (
        <output className="sr-only" aria-live="polite">
          {`${character.name}をクリップボードにコピーしました`}
        </output>
      )}
    </article>
  );
...
export const NewCharacterCard: FC = () => {
  ...
  return (
    ...
    >
      ...
      <span className="font-semibold">新しいキャラクター</span>
      <span className="text-xs text-muted-foreground">
        {canCreate ? '空のフォームから作成' : '空のフォームから作成(アカウント保存は上限)'}
      </span>
    </button>
  );
};
